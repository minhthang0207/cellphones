...
export async function middleware(req: NextRequest) {
  ...
  if (!token) {
    ...
    const isUserProtected = USER_PROTECTED.some((r) => url.pathname.startsWith(r));
    const isAdminProtected = ADMIN_PROTECTED.some((r) => url.pathname.startsWith(r));
  ...
  try {
    ...
    if (ADMIN_PROTECTED.some((r) => url.pathname.startsWith(r)) && payload.role !== "admin") {
      return NextResponse.redirect(new URL("/", req.url));
    }
